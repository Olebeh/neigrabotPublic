...
player.on('trackStart', (queue, track) => {
    ...
    .setDescription(`Зараз грає [${queue.current.title}](${queue.current.url}) [${track.requestedBy}]`)
...
player.on('trackAdd', (queue, track) => {
    ...
    .setColor('DARK_VIVID_PINK')
    .setDescription(`Трек [${track.title}](${track.url}) доданий в чергу! [${track.requestedBy}]`)
...
player.on('botDisconnect', (queue) => {
    ...
    .setColor('RED')
    .setDescription(`Від'єднано. Очищаю чергу...`)
...
player.on('queueEnd', (queue) => {
    ...
    .setColor('DARK_VIVID_PINK')
    .setDescription(`Чергу завершено!`)
